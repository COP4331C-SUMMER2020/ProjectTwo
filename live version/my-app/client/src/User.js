...
export default class User extends React.Component {
	...
	render() {
		const handleIngredients = async e => {
			...
			fetch('https://group-247-recipe.herokuapp.com/api/searchIngredients', {
				method: 'POST',
				headers: {
			...
			.then(data => {
				...
				if (data.error == "") {
					...
					document.getElementById("ingredientReturn").innerHTML = data.error;
					this.setState({ recipeResults: null });
			...
			.catch((error) => {
				...
				document.getElementById("ingredientReturn").innerHTML = error;
				this.setState({ recipeResults: null });
				this.setState({ ingredientResults: null });
				return;
			});
		...
		const handleRecipes = async e => {
			...
			fetch('https://group-247-recipe.herokuapp.com/api/searchRecipe', {
				method: 'POST',
				headers: {
					'Content-type': 'application/json',
				},
			...
			.then(data => {
				...
				if (data.error == "") {
					...
					document.getElementById("recipeReturn").innerHTML = data.error;
					this.setState({ recipeResults: null });
					this.setState({ ingredientResults: null });
				}
			})
			.catch((error) => {
				console.error('Error:', error);
				document.getElementById("recipeReturn").innerHTML = error;			
				this.setState({ recipeResults: null });
				this.setState({ ingredientResults: null });
				return;
			});
