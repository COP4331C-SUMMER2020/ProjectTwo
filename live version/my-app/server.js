 ...
 app.post('/api/register', async (req, res, next) =>
{
  ...
  else
  {
    try
    {
	  ...
	  const results = await db.collection('Users').find({email:email}).toArray();
	  id = results[0]._id;
	  const clink = 'https://group-247-recipe.herokuapp.com/confirm?id='+id;
	  //re-search for this user, send objectID with email
	  const msg = {
	  to: email,
      from: '24.7recipefinder@gmail.com',
      subject: 'Confirmation email',
      text: 'Please click the following link to verify your email: '+clink,
      };
	  sgMail.send(msg);
...
app.post('/api/email', async (req, res, next) => 
{
  ...
  {
	id = results[0]._id;
	const clink = 'https://group-247-recipe.herokuapp.com/reset?id='+id;
    const msg = {
      to: email,
        from: '24.7recipefinder@gmail.com',
        subject: 'Password Reset',
        text: 'Please click the following link to reset your password: '+clink,
        };
      sgMail.send(msg);
